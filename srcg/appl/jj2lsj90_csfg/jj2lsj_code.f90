...
!                                                                      *
!     Modify for CSFG version, Chongyang Chen, 2023                    *
!     Also some minor fixes labelled by CYC-2023                       *
!                                                                      *
!     Modified by G. Gaigalas                                   2024   *
!                                                                      *
!     Last modification for CSFG by C. Y. Chen              Aug 2024   *
!                                                                      *
!***********************************************************************
!-----------------------------------------------
!   M o d u l e s
...
!     Modified by G. Gaigalas                                   2024   *
!                                                                      *
!     Last modification for CSFG, C. Y. Chen,               Aug 2024   *
!***********************************************************************
!-----------------------------------------------
!   M o d u l e s
!-----------------------------------------------
...
!CYC-2024
!      CALL SETLABORB(TRIM(NAME)//'.l')
      call rcsfsymexpand(TRIM(NAME)//'.g', TRIM(NAME)//'.l', TRIM(NAME)//'.c', 0)
!
...
!CYC      USE ORB_C,           ONLY: NCF
      ...
      USE blk_C,           ONLY: NEVINBLK, NCFINBLK, NBLOCK, TWO_J
!CYC-2024
      USE symexpand_mod,   ONLY: nonsyminf, scfDF1, TotCSFs_perblock
!-----------------------------------------------
!   I n t e r f a c e   B l o c k s
!-----------------------------------------------
      USE itjpo_I
...
!-----------------------------------------------
      ...
      do  IBLKNUM = 1, NBLOCK
         ...
         ithresh = 0
!CYC-2024
         ! Ensure the CSFs are same from name.g and name.c files
         IF (NCFINBLK(IBLKNUM) /=  TotCSFs_perblock(3,IBLKNUM)) THEN
           WRITE(*,*)"Error: IBLKNUM, NCFINBLK(IBLKNUM),     &
                      TotCSFs_perblock(3,IBLKNUM) =", &
                      IBLKNUM, NCFINBLK(IBLKNUM),     &
                      TotCSFs_perblock(3,IBLKNUM)
           STOP "Error! NCFINBLK /=  TotCSFs_perblock(3,IBLKNUM) ..." 
         ENDIF
         NCFMAXtmp = NCFMIN + scfDF1(IBLKNUM) - 1
!         do  i = NCFMIN, NCFMAX
         do  i = NCFMIN, NCFMAXtmp
!CYC-2024 END
            sumthrsh = ZERO
...
!CYC-2023
!        Modifications for CSFGs expansion
!        To transfer the jj-CSFs with symmetry-ordered (non-labelling)
!        orbitals, the subroutines for transfermations should be
!        modified also using CSFGs.
!
!        By now, the transfermations are performed only for labelling
!        CSFs. The number of transfered CSFs are limited by NCFMAXtmp.
!
!        Thus, wb [Total sum over  weight (in LSJ)] might be far less
!        than 1.0d0. In such cases, the labeling orbital set should be
!        enlarged to inculde the important correlation CSFs, i.e., 
!        set all the orbitals with large generalised occupation numbers
!        as labeling ones. 
!
         NW = nonsyminf
!CYC-2023 end
         write(*,*)'The program generates a list of CSFs in LS-coupling'
!CYC-2024       call setLS(ithresh,NCFMIN,NCFMAX)
         call setLS(ithresh,NCFMIN,NCFMAXtmp)
!
!     output to *.lsj.c
...
!CYC-2023           (weights2(j),iw(j),J=1,5)
