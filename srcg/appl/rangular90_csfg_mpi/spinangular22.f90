!***********************************************************************
!                                                                      *
      SUBROUTINE SPINANGULAR22 (JASAV, JBSAV, IUNITF, NPOS)
!                                                                      *
...
! Construct IRFICT as the symmetry-ordered orbitals are not
! equal for JA/JB.
        ! < JB: (Core B) K | h1 | JA: (CoreA) I>: set K = I to accout
        ! for the possible one-body contributions. 

        ! JA: (Core A) 7s  --  JB: (Core B) 11s
        ! JB: (Core B) 11s ==> IRFICT: (Core B) 4s
        ! JA: (Core A) 7s  ==> ICFICT: (Core A) 4s
        ! Constructing the fictious CSFs with the lowest n value.
        CALL FICTIOUS_CSF(2, IRFICT, JB, NTYPE(3,JB), NTYPE(4,JB), 0, 0)
        CALL FICTIOUS_CSF(2, ICFICT, JA, NTYPE(3,JA), NTYPE(4,JA), 0, 0)
      !ENDIF
!
      ...
      DO 2 IV = 1, NVCOEF
         ...
         IF (ABS (VCOEFF) > CUTOFF) THEN
           ...
           !IF (.NOT. F0INT) THEN
             ...
             IF (NSYMCR == 0 .AND. (.NOT. LDIAG)) THEN
               ! Semi-diagonal matrixelement non-zero
               LDIAG = .TRUE.  
               DO I = 1, min(M,N)
                 LNonZero(I,I) = .TRUE.
               ENDDO
             ELSEIF (NSYMCR == 2 .AND. (.NOT. LOFFD)) THEN 
               ! Full matrix non-zero
               LOFFD = .TRUE.
