...
!CYC      USE where_C
      ...
      use symmatrix_restart_C,   ONLY: IFNCOL
!-----------------------------------------------
...
! Get iccutblk() from the user-input
      ...
      ! Let master do the i/o, then broadcast
         IF (myid .EQ. 0) THEN
            ...
            WRITE (istde,*) '  block     J Parity   No of CSFGs'// &
                            '    TrueCSFs'
            DO i = 1, nblock
               WRITE (istde,*) i, idblk(i)(1:5), ncfblk(i), NCFBLKTr(I)
            ENDDO

            WRITE (istde,*)
            WRITE (istde,*) 'Enter iccut (in CSFG list) for each block:'
            DO jblock = 1, nblock
!               WRITE (istde,*) 'Block ', jblock, '  ncfsym = ',       &
!                              ncfblk(jblock), NCFBLKTr(jblock)         &
...
! CYC:
      ...
      WRITE (IFNCOL, *) NELEC, NCFTOTTr, NW, nblock, &
                        "  =NELEC, NCF, NW, nblock"
!
