...
!CYC-2023/12/04
      !INTEGER(LONG), PARAMETER      :: NINCOR = 268435456 ! = 2 GB or  memory
      INTEGER(LONG)     :: NINCOR ! = 2147483648 ! 16 GB or  memory
...
! GG          DIATMP, CRITE, CRITC, CRITR, ORTHO, DMUNGO, AMAX, WA
      ...
      INTEGER :: NEVCOPY
      DOUBLE PRECISION, DIMENSION(:), ALLOCATABLE :: SCRA
...
! Set always SPARSE = .TRUE., determine LDISC using MaxMemPerProcs
            IF (MYID .EQ. 0) THEN
              ...
              WRITE(*,'(1x,a15, f10.2, a4, 1x, a)')"WSIZE (Total) =", &
                       WSIZE*NPROCS, ' GB,', &
              'which increase linearly with the cores used.'
...
!CYC-2024
!gfortran bug?                 CALL ALLOC (IENDC, NCF+1, 'IENDC', 'MANEIG')
!This is OK                 CALL ALLOC (IENDC, 0, NCF+1, 'IENDC', 'MANEIG')
!Or:
...
! Changed by CYC, Jun 2022
            IF (LCRITE) THEN
              ! Coarse calculation followed by rmixaccumulate_csfg, then
              ! perform another RCI calculation, using the condensed, /
              ! extended (expannded) CSFG expansion.
              CRITE = 1.0D-12
            ELSE
              CRITE = 1.0D-17
            ENDIF
...
!              IF (myid .EQ. 0) print *, 'Exit from iniestmpi ......'
              ...
              ! Sijie Wu and CYC: Reuse old.cm, and DEALLOCATE THE ARRAYS
              ! USED BY old.cm
              IF (LOLDCM) THEN
                NEVCOPY = MIN(NEVBLK_OLD, NVECMX)
                CALL DCOPY (NEVCOPY,EVAL_OLD,1,WORK(NEND+1),1)
                CALL DCOPY (NCFt*NEVCOPY,EVEC_OLD,1,WORK(1),1)
                IF (NVECMX .GT. NEVBLK_OLD) THEN
                  ALLOCATE (SCRA(NVECMX-NEVBLK_OLD))
                  Call mgs_nrm(NCFt, NEVBLK_OLD, NVECMX-NEVBLK_OLD, SCRA, WORK)
                  deallocate (SCRA)
                ENDIF
                deallocate (EVAL_OLD)
                deallocate (EVEC_OLD)
