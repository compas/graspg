...
!CYC      USE where_C
      ...
      use symmatrix_restart_C,  ONLY: RESTRT
!-----------------------------------------------
...
!
         ...
         nposition = 7 + nw + nw  ! File position of the previous block
                                  ! in the .res file
         ! In fact, the followings do not work for ONE block mode (jblock = 1).
!         DO i = 1, jblock - 1
!            j = (NCFblkTr(i) - myid - 1 + nprocs) / nprocs
...
!           ...eav, nelmnt are also obtained from genmat
            ...
            ! Retrun, use rdistHmatrix_csfg to diagonalize H-matrix by less
            ! Cores to save memory: the number of WORK-arrays in MANEIG.f90
            endtime = MPI_Wtime()
            if (myid.eq.0) THEN
              write(*,*)'Time for calculation of H matrix (m) = ',     &
...
!     ...Clean up
      ...
      IF (nvec > 0) CALL dalloc (iccmin, 'ICCMIN', 'MATRIX') ! allocated in items as pnccmin
      IF (.NOT. RESTRT) THEN
