...
!      USE mpi_C,   ONLY: ierr, MPI_COMM_WORLD, MPI_INTEGER
      USE mpi_C
!      USE MPI
      use symmatrix_restart_C,  ONLY: IFNCOL, ncsfDF1_core
!-----------------------------------------------
...
! Also used in lodres.f genmat.f
...
!Write the needed data into ncolsXXX.txt for RESTART calculation
      WRITE(IFNCOL, *)ncsfDF1, NELMNT, ncsfDF1_core, &
            "  = ncsfDF1, NELMNT,  ncsfDF1_core "
      WRITE(IFNCOL, *)NODENCOLS, "  = NODENCOLS"
      DO I = 1, NODENCOLS
        WRITE(IFNCOL, *)NODECOLS(I)
      ENDDO
      CLOSE(IFNCOL)
!      IF (MYIDDUMM.EQ.0) WRITE(*,*)"SETHAM: ELSTO = ", ELSTO
