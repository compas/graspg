...
!=======================================================================
      ...
      CALL startmpi2 (myid, nprocs, host, lenhost, ncount1,       &
                           startdir, permdir, tmpdir, 'RCI_CSFG_MPI')
...
!PERJ END
         ...
         NAMESAVE= permdir(1:lenperm) // '/' // NAME(1:lenname) // ' '
         isofile = permdir(1:lenperm) // '/isodata '
         FILE_RL = permdir(1:lenperm) // '/' // NAME(1:lenname) // '.l'
         FILE_OUT= permdir(1:lenperm) // '/' // NAME(1:lenname) // '.c'
         NAME    = permdir(1:lenperm) // '/' // NAME(1:lenname) // '.g'
...
!         write (*,*)'Calling rcsfsymexpand .....'
         call rcsfsymexpand(NAME, FILE_RL, FILE_OUT, 1)
         DEALLOCATE(MAP2)
      ENDIF
!=======================================================================
...
!
      IF (LTRANS) THEN
         ...
         endtime = MPI_Wtime()
         if (myid.eq.0)                                                &
           print *, 'Time for genintbreit1 calculation (s) = ',        &
                  endtime-starttime
         starttime = endtime
         CALL GENINTBREIT2WRAP (myid, nprocs, j2max)
         endtime = MPI_Wtime()
         if (myid.eq.0)                                                &
           print *, 'Time for genintbreit2 calculation (s) = ',        &
                  endtime-starttime
      END IF
!=======================================================================
