...
! n = 16
                                    ng16 = 16
!     16s
                                    call sluggo (ng16, 0, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,0), low, start(ng16&
                                       ,0), stopp(ng16,0))
                                    do ang0 = start(ng16,0), stopp(ng16,0), -1
...
!     16p
                                    call sluggo (ng16, 1, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,1), low, start(ng16&
                                       ,1), stopp(ng16,1))
                                    do ang1 = start(ng16,1), stopp(ng16,1), -1
                                    antel(ng16,1) = ang1 + antel(ng16,0)
                                    if (antel(ng16,1) > antal) cycle
                                    do plusg1 = min(ang1,4), max(ang1 - 2,0), &
                                       -1
                                    ansats(ng16,1,1) = plusg1
                                    ansats(ng16,1,0) = ang1 - plusg1
!     16d
                                    call sluggo (ng16, 2, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,2), low, start(ng16&
                                       ,2), stopp(ng16,2))
                                    do ang2 = start(ng16,2), stopp(ng16,2), -1
                                    antel(ng16,2) = ang2 + antel(ng16,1)
                                    if (antel(ng16,2) > antal) cycle
                                    do plusg2 = min(ang2,6), max(ang2 - 4,0), &
                                       -1
                                    ansats(ng16,2,1) = plusg2
                                    ansats(ng16,2,0) = ang2 - plusg2
!     16f
                                    call sluggo (ng16, 3, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,3), low, start(ng16&
                                       ,3), stopp(ng16,3))
                                    do ang3 = start(ng16,3), stopp(ng16,3), -1
                                    antel(ng16,3) = ang3 + antel(ng16,2)
                                    if (antel(ng16,3) > antal) cycle
                                    do plusg3 = min(ang3,8), max(ang3 - 6,0), &
                                       -1
                                    ansats(ng16,3,1) = plusg3
                                    ansats(ng16,3,0) = ang3 - plusg3
!     16g
                                    call sluggo (ng16, 4, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,4), low, start(ng16&
                                       ,4), stopp(ng16,4))
                                    do ang4 = start(ng16,4), stopp(ng16,4), -1
                                    antel(ng16,4) = ang4 + antel(ng16,3)
                                    if (antel(ng16,4) > antal) cycle
                                    do plusg4 = min(ang4,10), max(ang4 - 8,0), &
                                       -1
                                    ansats(ng16,4,1) = plusg4
                                    ansats(ng16,4,0) = ang4 - plusg4
!     16h
                                    call sluggo (ng16, 5, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,5), low, start(ng16&
                                       ,5), stopp(ng16,5))
                                    do ang5 = start(ng16,5), stopp(ng16,5), -1
                                    antel(ng16,5) = ang5 + antel(ng16,4)
                                    if (antel(ng16,5)>antal .or. ansats(ng16,4,1)>2&
                                       ) cycle
                                    do plusg5 = min(ang5,12), max(ang5 - 10,0)&
                                       , -1
                                    ansats(ng16,5,1) = plusg5
                                    ansats(ng16,5,0) = ang5 - plusg5
!     16i
                                    call sluggo (ng16, 6, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,6), low, start(ng16&
                                       ,6), stopp(ng16,6))
                                    do ang6 = start(ng16,6), stopp(ng16,6), -1
                                    antel(ng16,6) = ang6 + antel(ng16,5)
                                    if (.not.(antel(ng16,6)<=antal .and. ansats(&
                                       ng16,5,1)<=2 .and. ansats(ng16,5,0)<=2)) &
                                       cycle
                                    do plusg6 = min(ang6,14), max(ang6 - 12,0)&
                                       , -1
                                    ansats(ng16,6,1) = plusg6
                                    ansats(ng16,6,0) = ang6 - plusg6
!     16k
                                    call sluggo (ng16, 7, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,7), low, start(ng16&
                                       ,7), stopp(ng16,7))
                                    do ang7 = start(ng16,7), stopp(ng16,7), -1
                                    antel(ng16,7) = ang7 + antel(ng16,6)
                                    if (.not.(antel(ng16,7)<=antal .and. ansats(&
                                       ng16,6,1)<=2 .and. ansats(ng16,6,0)<=2)) &
                                       cycle
                                    do plusg7 = min(ang7,16), max(ang7 - 14,0)&
                                       , -1
                                    ansats(ng16,7,1) = plusg7
                                    ansats(ng16,7,0) = ang7 - plusg7
!     16l
                                    call sluggo (ng16, 8, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,8), low, start(ng16&
                                       ,8), stopp(ng16,8))
                                    do ang8 = start(ng16,8), stopp(ng16,8), -1
                                    antel(ng16,8) = ang8 + antel(ng16,7)
                                    if (.not.(antel(ng16,8)<=antal .and. ansats(&
                                       ng16,7,1)<=2 .and. ansats(ng16,7,0)<=2)) &
                                       cycle
                                    do plusg8 = min(ang8,18), max(ang8 - 16,0)&
                                       , -1
                                    ansats(ng16,8,1) = plusg8
                                    ansats(ng16,8,0) = ang8 - plusg8
!     16m
                                    call sluggo (ng16, 9, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,9), low, start(ng16&
                                       ,9), stopp(ng16,9))
                                    do ang9 = start(ng16,9), stopp(ng16,9), -1
                                    antel(ng16,9) = ang9 + antel(ng16,8)
                                    if (.not.(antel(ng16,9)<=antal .and. ansats(&
                                       ng16,8,1)<=2 .and. ansats(ng16,8,0)<=2)) &
                                       cycle
                                    do plusg9 = min(ang9,20), max(ang9 - 18,0)&
                                       , -1
                                    ansats(ng16,9,1) = plusg9
                                    ansats(ng16,9,0) = ang9 - plusg9
!     16n
                                    call sluggo (ng16, 10, varmax, varupp, varned&
                                       , ansats, org, lock(ng16,10), low, start(&
                                       ng16,10), stopp(ng16,10))
                                    do anga = start(ng16,10), stopp(ng16,10), -1
                                    antel(ng16,10) = anga + antel(ng16,9)
                                    !if (.not.(antel(ng16,10)==antal .and. ansats(&
                                    if (.not.(antel(ng16,10)<=antal .and. ansats(&
                                       ng16,9,1)<=2 .and. ansats(ng16,9,0)<=2)) &
                                       cycle
                                    do plusga = min(anga,22), max(anga - 20,0)&
                                       , -1
...
! n = 17
                                    nh17 = 17
!     17s
                                    call sluggo (nh17, 0, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,0), low, start(nh17&
                                       ,0), stopp(nh17,0))
                                    do anh0 = start(nh17,0), stopp(nh17,0), -1
...
!     17p
                                    call sluggo (nh17, 1, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,1), low, start(nh17&
                                       ,1), stopp(nh17,1))
                                    do anh1 = start(nh17,1), stopp(nh17,1), -1
                                    antel(nh17,1) = anh1 + antel(nh17,0)
                                    if (antel(nh17,1) > antal) cycle
                                    do plush1 = min(anh1,4), max(anh1 - 2,0), &
                                       -1
                                    ansats(nh17,1,1) = plush1
                                    ansats(nh17,1,0) = anh1 - plush1
!     17d
                                    call sluggo (nh17, 2, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,2), low, start(nh17&
                                       ,2), stopp(nh17,2))
                                    do anh2 = start(nh17,2), stopp(nh17,2), -1
                                    antel(nh17,2) = anh2 + antel(nh17,1)
                                    if (antel(nh17,2) > antal) cycle
                                    do plush2 = min(anh2,6), max(anh2 - 4,0), &
                                       -1
                                    ansats(nh17,2,1) = plush2
                                    ansats(nh17,2,0) = anh2 - plush2
!     17f
                                    call sluggo (nh17, 3, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,3), low, start(nh17&
                                       ,3), stopp(nh17,3))
                                    do anh3 = start(nh17,3), stopp(nh17,3), -1
                                    antel(nh17,3) = anh3 + antel(nh17,2)
                                    if (antel(nh17,3) > antal) cycle
                                    do plush3 = min(anh3,8), max(anh3 - 6,0), &
                                       -1
                                    ansats(nh17,3,1) = plush3
                                    ansats(nh17,3,0) = anh3 - plush3
!     17g
                                    call sluggo (nh17, 4, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,4), low, start(nh17&
                                       ,4), stopp(nh17,4))
                                    do anh4 = start(nh17,4), stopp(nh17,4), -1
                                    antel(nh17,4) = anh4 + antel(nh17,3)
                                    if (antel(nh17,4) > antal) cycle
                                    do plush4 = min(anh4,10), max(anh4 - 8,0), &
                                       -1
                                    ansats(nh17,4,1) = plush4
                                    ansats(nh17,4,0) = anh4 - plush4
!     17h
                                    call sluggo (nh17, 5, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,5), low, start(nh17&
                                       ,5), stopp(nh17,5))
                                    do anh5 = start(nh17,5), stopp(nh17,5), -1
                                    antel(nh17,5) = anh5 + antel(nh17,4)
                                    if (antel(nh17,5)>antal .or. ansats(nh17,4,1)>2&
                                       ) cycle
                                    do plush5 = min(anh5,12), max(anh5 - 10,0)&
                                       , -1
                                    ansats(nh17,5,1) = plush5
                                    ansats(nh17,5,0) = anh5 - plush5
!     17i
                                    call sluggo (nh17, 6, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,6), low, start(nh17&
                                       ,6), stopp(nh17,6))
                                    do anh6 = start(nh17,6), stopp(nh17,6), -1
                                    antel(nh17,6) = anh6 + antel(nh17,5)
                                    if (.not.(antel(nh17,6)<=antal .and. ansats(&
                                       nh17,5,1)<=2 .and. ansats(nh17,5,0)<=2)) &
                                       cycle
                                    do plush6 = min(anh6,14), max(anh6 - 12,0)&
                                       , -1
                                    ansats(nh17,6,1) = plush6
                                    ansats(nh17,6,0) = anh6 - plush6
!     17k
                                    call sluggo (nh17, 7, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,7), low, start(nh17&
                                       ,7), stopp(nh17,7))
                                    do anh7 = start(nh17,7), stopp(nh17,7), -1
                                    antel(nh17,7) = anh7 + antel(nh17,6)
                                    if (.not.(antel(nh17,7)<=antal .and. ansats(&
                                       nh17,6,1)<=2 .and. ansats(nh17,6,0)<=2)) &
                                       cycle
                                    do plush7 = min(anh7,16), max(anh7 - 14,0)&
                                       , -1
                                    ansats(nh17,7,1) = plush7
                                    ansats(nh17,7,0) = anh7 - plush7
!     17l
                                    call sluggo (nh17, 8, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,8), low, start(nh17&
                                       ,8), stopp(nh17,8))
                                    do anh8 = start(nh17,8), stopp(nh17,8), -1
                                    antel(nh17,8) = anh8 + antel(nh17,7)
                                    if (.not.(antel(nh17,8)<=antal .and. ansats(&
                                       nh17,7,1)<=2 .and. ansats(nh17,7,0)<=2)) &
                                       cycle
                                    do plush8 = min(anh8,18), max(anh8 - 16,0)&
                                       , -1
                                    ansats(nh17,8,1) = plush8
                                    ansats(nh17,8,0) = anh8 - plush8
!     17m
                                    call sluggo (nh17, 9, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,9), low, start(nh17&
                                       ,9), stopp(nh17,9))
                                    do anh9 = start(nh17,9), stopp(nh17,9), -1
                                    antel(nh17,9) = anh9 + antel(nh17,8)
                                    if (.not.(antel(nh17,9)<=antal .and. ansats(&
                                       nh17,8,1)<=2 .and. ansats(nh17,8,0)<=2)) &
                                       cycle
                                    do plush9 = min(anh9,20), max(anh9 - 18,0)&
                                       , -1
                                    ansats(nh17,9,1) = plush9
                                    ansats(nh17,9,0) = anh9 - plush9
!     17n
                                    call sluggo (nh17, 10, varmax, varupp, varned&
                                       , ansats, org, lock(nh17,10), low, start(&
                                       nh17,10), stopp(nh17,10))
                                    do anha = start(nh17,10), stopp(nh17,10), -1
                                    antel(nh17,10) = anha + antel(nh17,9)
                                    !if (.not.(antel(nh17,10)==antal .and. ansats(&
                                    if (.not.(antel(nh17,10)<=antal .and. ansats(&
                                       nh17,9,1)<=2 .and. ansats(nh17,9,0)<=2)) &
                                       cycle
                                    do plusha = min(anha,22), max(anha - 20,0)&
                                       , -1
...
! n = 18
                                    ni18 = 18
!     18s
                                    call sluggo (ni18, 0, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,0), low, start(ni18&
                                       ,0), stopp(ni18,0))
                                    do ani0 = start(ni18,0), stopp(ni18,0), -1
...
!     18p
                                    call sluggo (ni18, 1, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,1), low, start(ni18&
                                       ,1), stopp(ni18,1))
                                    do ani1 = start(ni18,1), stopp(ni18,1), -1
                                    antel(ni18,1) = ani1 + antel(ni18,0)
                                    if (antel(ni18,1) > antal) cycle
                                    do plusi1 = min(ani1,4), max(ani1 - 2,0), &
                                       -1
                                    ansats(ni18,1,1) = plusi1
                                    ansats(ni18,1,0) = ani1 - plusi1
!     18d
                                    call sluggo (ni18, 2, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,2), low, start(ni18&
                                       ,2), stopp(ni18,2))
                                    do ani2 = start(ni18,2), stopp(ni18,2), -1
                                    antel(ni18,2) = ani2 + antel(ni18,1)
                                    if (antel(ni18,2) > antal) cycle
                                    do plusi2 = min(ani2,6), max(ani2 - 4,0), &
                                       -1
                                    ansats(ni18,2,1) = plusi2
                                    ansats(ni18,2,0) = ani2 - plusi2
!     18f
                                    call sluggo (ni18, 3, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,3), low, start(ni18&
                                       ,3), stopp(ni18,3))
                                    do ani3 = start(ni18,3), stopp(ni18,3), -1
                                    antel(ni18,3) = ani3 + antel(ni18,2)
                                    if (antel(ni18,3) > antal) cycle
                                    do plusi3 = min(ani3,8), max(ani3 - 6,0), &
                                       -1
                                    ansats(ni18,3,1) = plusi3
                                    ansats(ni18,3,0) = ani3 - plusi3
!     18g
                                    call sluggo (ni18, 4, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,4), low, start(ni18&
                                       ,4), stopp(ni18,4))
                                    do ani4 = start(ni18,4), stopp(ni18,4), -1
                                    antel(ni18,4) = ani4 + antel(ni18,3)
                                    if (antel(ni18,4) > antal) cycle
                                    do plusi4 = min(ani4,10), max(ani4 - 8,0), &
                                       -1
                                    ansats(ni18,4,1) = plusi4
                                    ansats(ni18,4,0) = ani4 - plusi4
!     18h
                                    call sluggo (ni18, 5, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,5), low, start(ni18&
                                       ,5), stopp(ni18,5))
                                    do ani5 = start(ni18,5), stopp(ni18,5), -1
                                    antel(ni18,5) = ani5 + antel(ni18,4)
                                    if (antel(ni18,5)>antal .or. ansats(ni18,4,1)>2&
                                       ) cycle
                                    do plusi5 = min(ani5,12), max(ani5 - 10,0)&
                                       , -1
                                    ansats(ni18,5,1) = plusi5
                                    ansats(ni18,5,0) = ani5 - plusi5
!     18i
                                    call sluggo (ni18, 6, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,6), low, start(ni18&
                                       ,6), stopp(ni18,6))
                                    do ani6 = start(ni18,6), stopp(ni18,6), -1
                                    antel(ni18,6) = ani6 + antel(ni18,5)
                                    if (.not.(antel(ni18,6)<=antal .and. ansats(&
                                       ni18,5,1)<=2 .and. ansats(ni18,5,0)<=2)) &
                                       cycle
                                    do plusi6 = min(ani6,14), max(ani6 - 12,0)&
                                       , -1
                                    ansats(ni18,6,1) = plusi6
                                    ansats(ni18,6,0) = ani6 - plusi6
!     18k
                                    call sluggo (ni18, 7, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,7), low, start(ni18&
                                       ,7), stopp(ni18,7))
                                    do ani7 = start(ni18,7), stopp(ni18,7), -1
                                    antel(ni18,7) = ani7 + antel(ni18,6)
                                    if (.not.(antel(ni18,7)<=antal .and. ansats(&
                                       ni18,6,1)<=2 .and. ansats(ni18,6,0)<=2)) &
                                       cycle
                                    do plusi7 = min(ani7,16), max(ani7 - 14,0)&
                                       , -1
                                    ansats(ni18,7,1) = plusi7
                                    ansats(ni18,7,0) = ani7 - plusi7
!     18l
                                    call sluggo (ni18, 8, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,8), low, start(ni18&
                                       ,8), stopp(ni18,8))
                                    do ani8 = start(ni18,8), stopp(ni18,8), -1
                                    antel(ni18,8) = ani8 + antel(ni18,7)
                                    if (.not.(antel(ni18,8)<=antal .and. ansats(&
                                       ni18,7,1)<=2 .and. ansats(ni18,7,0)<=2)) &
                                       cycle
                                    do plusi8 = min(ani8,18), max(ani8 - 16,0)&
                                       , -1
                                    ansats(ni18,8,1) = plusi8
                                    ansats(ni18,8,0) = ani8 - plusi8
!     18m
                                    call sluggo (ni18, 9, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,9), low, start(ni18&
                                       ,9), stopp(ni18,9))
                                    do ani9 = start(ni18,9), stopp(ni18,9), -1
                                    antel(ni18,9) = ani9 + antel(ni18,8)
                                    if (.not.(antel(ni18,9)<=antal .and. ansats(&
                                       ni18,8,1)<=2 .and. ansats(ni18,8,0)<=2)) &
                                       cycle
                                    do plusi9 = min(ani9,20), max(ani9 - 18,0)&
                                       , -1
                                    ansats(ni18,9,1) = plusi9
                                    ansats(ni18,9,0) = ani9 - plusi9
!     18n
                                    call sluggo (ni18, 10, varmax, varupp, varned&
                                       , ansats, org, lock(ni18,10), low, start(&
                                       ni18,10), stopp(ni18,10))
                                    do ania = start(ni18,10), stopp(ni18,10), -1
                                    antel(ni18,10) = ania + antel(ni18,9)
                                    !if (.not.(antel(ni18,10)==antal .and. ansats(&
                                    if (.not.(antel(ni18,10)<=antal .and. ansats(&
                                       ni18,9,1)<=2 .and. ansats(ni18,9,0)<=2)) &
                                       cycle
                                    do plusia = min(ania,22), max(ania - 20,0)&
                                       , -1
...
! n = 19
                                    nj19 = 19
!     19s
                                    call sluggo (nj19, 0, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,0), low, start(nj19&
                                       ,0), stopp(nj19,0))
                                    do anj0 = start(nj19,0), stopp(nj19,0), -1
...
!     19p
                                    call sluggo (nj19, 1, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,1), low, start(nj19&
                                       ,1), stopp(nj19,1))
                                    do anj1 = start(nj19,1), stopp(nj19,1), -1
                                    antel(nj19,1) = anj1 + antel(nj19,0)
                                    if (antel(nj19,1) > antal) cycle
                                    do plusj1 = min(anj1,4), max(anj1 - 2,0), &
                                       -1
                                    ansats(nj19,1,1) = plusj1
                                    ansats(nj19,1,0) = anj1 - plusj1
!     19d
                                    call sluggo (nj19, 2, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,2), low, start(nj19&
                                       ,2), stopp(nj19,2))
                                    do anj2 = start(nj19,2), stopp(nj19,2), -1
                                    antel(nj19,2) = anj2 + antel(nj19,1)
                                    if (antel(nj19,2) > antal) cycle
                                    do plusj2 = min(anj2,6), max(anj2 - 4,0), &
                                       -1
                                    ansats(nj19,2,1) = plusj2
                                    ansats(nj19,2,0) = anj2 - plusj2
!     19f
                                    call sluggo (nj19, 3, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,3), low, start(nj19&
                                       ,3), stopp(nj19,3))
                                    do anj3 = start(nj19,3), stopp(nj19,3), -1
                                    antel(nj19,3) = anj3 + antel(nj19,2)
                                    if (antel(nj19,3) > antal) cycle
                                    do plusj3 = min(anj3,8), max(anj3 - 6,0), &
                                       -1
                                    ansats(nj19,3,1) = plusj3
                                    ansats(nj19,3,0) = anj3 - plusj3
!     19g
                                    call sluggo (nj19, 4, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,4), low, start(nj19&
                                       ,4), stopp(nj19,4))
                                    do anj4 = start(nj19,4), stopp(nj19,4), -1
                                    antel(nj19,4) = anj4 + antel(nj19,3)
                                    if (antel(nj19,4) > antal) cycle
                                    do plusj4 = min(anj4,10), max(anj4 - 8,0), &
                                       -1
                                    ansats(nj19,4,1) = plusj4
                                    ansats(nj19,4,0) = anj4 - plusj4
!     19h
                                    call sluggo (nj19, 5, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,5), low, start(nj19&
                                       ,5), stopp(nj19,5))
                                    do anj5 = start(nj19,5), stopp(nj19,5), -1
                                    antel(nj19,5) = anj5 + antel(nj19,4)
                                    if (antel(nj19,5)>antal .or. ansats(nj19,4,1)>2&
                                       ) cycle
                                    do plusj5 = min(anj5,12), max(anj5 - 10,0)&
                                       , -1
                                    ansats(nj19,5,1) = plusj5
                                    ansats(nj19,5,0) = anj5 - plusj5
!     19i
                                    call sluggo (nj19, 6, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,6), low, start(nj19&
                                       ,6), stopp(nj19,6))
                                    do anj6 = start(nj19,6), stopp(nj19,6), -1
                                    antel(nj19,6) = anj6 + antel(nj19,5)
                                    if (.not.(antel(nj19,6)<=antal .and. ansats(&
                                       nj19,5,1)<=2 .and. ansats(nj19,5,0)<=2)) &
                                       cycle
                                    do plusj6 = min(anj6,14), max(anj6 - 12,0)&
                                       , -1
                                    ansats(nj19,6,1) = plusj6
                                    ansats(nj19,6,0) = anj6 - plusj6
!     19k
                                    call sluggo (nj19, 7, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,7), low, start(nj19&
                                       ,7), stopp(nj19,7))
                                    do anj7 = start(nj19,7), stopp(nj19,7), -1
                                    antel(nj19,7) = anj7 + antel(nj19,6)
                                    if (.not.(antel(nj19,7)<=antal .and. ansats(&
                                       nj19,6,1)<=2 .and. ansats(nj19,6,0)<=2)) &
                                       cycle
                                    do plusj7 = min(anj7,16), max(anj7 - 14,0)&
                                       , -1
                                    ansats(nj19,7,1) = plusj7
                                    ansats(nj19,7,0) = anj7 - plusj7
!     19l
                                    call sluggo (nj19, 8, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,8), low, start(nj19&
                                       ,8), stopp(nj19,8))
                                    do anj8 = start(nj19,8), stopp(nj19,8), -1
                                    antel(nj19,8) = anj8 + antel(nj19,7)
                                    if (.not.(antel(nj19,8)<=antal .and. ansats(&
                                       nj19,7,1)<=2 .and. ansats(nj19,7,0)<=2)) &
                                       cycle
                                    do plusj8 = min(anj8,18), max(anj8 - 16,0)&
                                       , -1
                                    ansats(nj19,8,1) = plusj8
                                    ansats(nj19,8,0) = anj8 - plusj8
!     19m
                                    call sluggo (nj19, 9, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,9), low, start(nj19&
                                       ,9), stopp(nj19,9))
                                    do anj9 = start(nj19,9), stopp(nj19,9), -1
                                    antel(nj19,9) = anj9 + antel(nj19,8)
                                    if (.not.(antel(nj19,9)<=antal .and. ansats(&
                                       nj19,8,1)<=2 .and. ansats(nj19,8,0)<=2)) &
                                       cycle
                                    do plusj9 = min(anj9,20), max(anj9 - 18,0)&
                                       , -1
                                    ansats(nj19,9,1) = plusj9
                                    ansats(nj19,9,0) = anj9 - plusj9
!     19n
                                    call sluggo (nj19, 10, varmax, varupp, varned&
                                       , ansats, org, lock(nj19,10), low, start(&
                                       nj19,10), stopp(nj19,10))
                                    do anja = start(nj19,10), stopp(nj19,10), -1
                                    antel(nj19,10) = anja + antel(nj19,9)
                                    !if (.not.(antel(nj19,10)==antal .and. ansats(&
                                    if (.not.(antel(nj19,10)<=antal .and. ansats(&
                                       nj19,9,1)<=2 .and. ansats(nj19,9,0)<=2)) &
                                       cycle
                                    do plusja = min(anja,22), max(anja - 20,0)&
                                       , -1
...
! n = 20
                                    nk20 = 20
!     20s
                                    call sluggo (nk20, 0, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,0), low, start(nk20&
                                       ,0), stopp(nk20,0))
                                    do ank0 = start(nk20,0), stopp(nk20,0), -1
...
!     20p
                                    call sluggo (nk20, 1, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,1), low, start(nk20&
                                       ,1), stopp(nk20,1))
                                    do ank1 = start(nk20,1), stopp(nk20,1), -1
                                    antel(nk20,1) = ank1 + antel(nk20,0)
                                    if (antel(nk20,1) > antal) cycle
                                    do plusk1 = min(ank1,4), max(ank1 - 2,0), &
                                       -1
                                    ansats(nk20,1,1) = plusk1
                                    ansats(nk20,1,0) = ank1 - plusk1
!     20d
                                    call sluggo (nk20, 2, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,2), low, start(nk20&
                                       ,2), stopp(nk20,2))
                                    do ank2 = start(nk20,2), stopp(nk20,2), -1
                                    antel(nk20,2) = ank2 + antel(nk20,1)
                                    if (antel(nk20,2) > antal) cycle
                                    do plusk2 = min(ank2,6), max(ank2 - 4,0), &
                                       -1
                                    ansats(nk20,2,1) = plusk2
                                    ansats(nk20,2,0) = ank2 - plusk2
!     20f
                                    call sluggo (nk20, 3, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,3), low, start(nk20&
                                       ,3), stopp(nk20,3))
                                    do ank3 = start(nk20,3), stopp(nk20,3), -1
                                    antel(nk20,3) = ank3 + antel(nk20,2)
                                    if (antel(nk20,3) > antal) cycle
                                    do plusk3 = min(ank3,8), max(ank3 - 6,0), &
                                       -1
                                    ansats(nk20,3,1) = plusk3
                                    ansats(nk20,3,0) = ank3 - plusk3
!     20g
                                    call sluggo (nk20, 4, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,4), low, start(nk20&
                                       ,4), stopp(nk20,4))
                                    do ank4 = start(nk20,4), stopp(nk20,4), -1
                                    antel(nk20,4) = ank4 + antel(nk20,3)
                                    if (antel(nk20,4) > antal) cycle
                                    do plusk4 = min(ank4,10), max(ank4 - 8,0), &
                                       -1
                                    ansats(nk20,4,1) = plusk4
                                    ansats(nk20,4,0) = ank4 - plusk4
!     20h
                                    call sluggo (nk20, 5, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,5), low, start(nk20&
                                       ,5), stopp(nk20,5))
                                    do ank5 = start(nk20,5), stopp(nk20,5), -1
                                    antel(nk20,5) = ank5 + antel(nk20,4)
                                    if (antel(nk20,5)>antal .or. ansats(nk20,4,1)>2&
                                       ) cycle
                                    do plusk5 = min(ank5,12), max(ank5 - 10,0)&
                                       , -1
                                    ansats(nk20,5,1) = plusk5
                                    ansats(nk20,5,0) = ank5 - plusk5
!     20i
                                    call sluggo (nk20, 6, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,6), low, start(nk20&
                                       ,6), stopp(nk20,6))
                                    do ank6 = start(nk20,6), stopp(nk20,6), -1
                                    antel(nk20,6) = ank6 + antel(nk20,5)
                                    if (.not.(antel(nk20,6)<=antal .and. ansats(&
                                       nk20,5,1)<=2 .and. ansats(nk20,5,0)<=2)) &
                                       cycle
                                    do plusk6 = min(ank6,14), max(ank6 - 12,0)&
                                       , -1
                                    ansats(nk20,6,1) = plusk6
                                    ansats(nk20,6,0) = ank6 - plusk6
!     20k
                                    call sluggo (nk20, 7, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,7), low, start(nk20&
                                       ,7), stopp(nk20,7))
                                    do ank7 = start(nk20,7), stopp(nk20,7), -1
                                    antel(nk20,7) = ank7 + antel(nk20,6)
                                    if (.not.(antel(nk20,7)<=antal .and. ansats(&
                                       nk20,6,1)<=2 .and. ansats(nk20,6,0)<=2)) &
                                       cycle
                                    do plusk7 = min(ank7,16), max(ank7 - 14,0)&
                                       , -1
                                    ansats(nk20,7,1) = plusk7
                                    ansats(nk20,7,0) = ank7 - plusk7
!     20l
                                    call sluggo (nk20, 8, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,8), low, start(nk20&
                                       ,8), stopp(nk20,8))
                                    do ank8 = start(nk20,8), stopp(nk20,8), -1
                                    antel(nk20,8) = ank8 + antel(nk20,7)
                                    if (.not.(antel(nk20,8)<=antal .and. ansats(&
                                       nk20,7,1)<=2 .and. ansats(nk20,7,0)<=2)) &
                                       cycle
                                    do plusk8 = min(ank8,18), max(ank8 - 16,0)&
                                       , -1
                                    ansats(nk20,8,1) = plusk8
                                    ansats(nk20,8,0) = ank8 - plusk8
!     20m
                                    call sluggo (nk20, 9, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,9), low, start(nk20&
                                       ,9), stopp(nk20,9))
                                    do ank9 = start(nk20,9), stopp(nk20,9), -1
                                    antel(nk20,9) = ank9 + antel(nk20,8)
                                    if (.not.(antel(nk20,9)<=antal .and. ansats(&
                                       nk20,8,1)<=2 .and. ansats(nk20,8,0)<=2)) &
                                       cycle
                                    do plusk9 = min(ank9,20), max(ank9 - 18,0)&
                                       , -1
                                    ansats(nk20,9,1) = plusk9
                                    ansats(nk20,9,0) = ank9 - plusk9
!     20n
                                    call sluggo (nk20, 10, varmax, varupp, varned&
                                       , ansats, org, lock(nk20,10), low, start(&
                                       nk20,10), stopp(nk20,10))
                                    do anka = start(nk20,10), stopp(nk20,10), -1
                                    antel(nk20,10) = anka + antel(nk20,9)
                                    !if (.not.(antel(nk20,10)==antal .and. ansats(&
                                    if (.not.(antel(nk20,10)<=antal .and. ansats(&
                                       nk20,9,1)<=2 .and. ansats(nk20,9,0)<=2)) &
                                       cycle
                                    do pluska = min(anka,22), max(anka - 20,0)&
                                       , -1
...
! n = 21
                                    nl21 = 21
!     21s
                                    call sluggo (nl21, 0, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,0), low, start(nl21&
                                       ,0), stopp(nl21,0))
                                    do anl0 = start(nl21,0), stopp(nl21,0), -1
...
!     21p
                                    call sluggo (nl21, 1, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,1), low, start(nl21&
                                       ,1), stopp(nl21,1))
                                    do anl1 = start(nl21,1), stopp(nl21,1), -1
                                    antel(nl21,1) = anl1 + antel(nl21,0)
                                    if (antel(nl21,1) > antal) cycle
                                    do plusl1 = min(anl1,4), max(anl1 - 2,0), &
                                       -1
                                    ansats(nl21,1,1) = plusl1
                                    ansats(nl21,1,0) = anl1 - plusl1
!     21d
                                    call sluggo (nl21, 2, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,2), low, start(nl21&
                                       ,2), stopp(nl21,2))
                                    do anl2 = start(nl21,2), stopp(nl21,2), -1
                                    antel(nl21,2) = anl2 + antel(nl21,1)
                                    if (antel(nl21,2) > antal) cycle
                                    do plusl2 = min(anl2,6), max(anl2 - 4,0), &
                                       -1
                                    ansats(nl21,2,1) = plusl2
                                    ansats(nl21,2,0) = anl2 - plusl2
!     21f
                                    call sluggo (nl21, 3, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,3), low, start(nl21&
                                       ,3), stopp(nl21,3))
                                    do anl3 = start(nl21,3), stopp(nl21,3), -1
                                    antel(nl21,3) = anl3 + antel(nl21,2)
                                    if (antel(nl21,3) > antal) cycle
                                    do plusl3 = min(anl3,8), max(anl3 - 6,0), &
                                       -1
                                    ansats(nl21,3,1) = plusl3
                                    ansats(nl21,3,0) = anl3 - plusl3
!     21g
                                    call sluggo (nl21, 4, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,4), low, start(nl21&
                                       ,4), stopp(nl21,4))
                                    do anl4 = start(nl21,4), stopp(nl21,4), -1
                                    antel(nl21,4) = anl4 + antel(nl21,3)
                                    if (antel(nl21,4) > antal) cycle
                                    do plusl4 = min(anl4,10), max(anl4 - 8,0), &
                                       -1
                                    ansats(nl21,4,1) = plusl4
                                    ansats(nl21,4,0) = anl4 - plusl4
!     21h
                                    call sluggo (nl21, 5, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,5), low, start(nl21&
                                       ,5), stopp(nl21,5))
                                    do anl5 = start(nl21,5), stopp(nl21,5), -1
                                    antel(nl21,5) = anl5 + antel(nl21,4)
                                    if (antel(nl21,5)>antal .or. ansats(nl21,4,1)>2&
                                       ) cycle
                                    do plusl5 = min(anl5,12), max(anl5 - 10,0)&
                                       , -1
                                    ansats(nl21,5,1) = plusl5
                                    ansats(nl21,5,0) = anl5 - plusl5
!     21i
                                    call sluggo (nl21, 6, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,6), low, start(nl21&
                                       ,6), stopp(nl21,6))
                                    do anl6 = start(nl21,6), stopp(nl21,6), -1
                                    antel(nl21,6) = anl6 + antel(nl21,5)
                                    if (.not.(antel(nl21,6)<=antal .and. ansats(&
                                       nl21,5,1)<=2 .and. ansats(nl21,5,0)<=2)) &
                                       cycle
                                    do plusl6 = min(anl6,14), max(anl6 - 12,0)&
                                       , -1
                                    ansats(nl21,6,1) = plusl6
                                    ansats(nl21,6,0) = anl6 - plusl6
!     21k
                                    call sluggo (nl21, 7, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,7), low, start(nl21&
                                       ,7), stopp(nl21,7))
                                    do anl7 = start(nl21,7), stopp(nl21,7), -1
                                    antel(nl21,7) = anl7 + antel(nl21,6)
                                    if (.not.(antel(nl21,7)<=antal .and. ansats(&
                                       nl21,6,1)<=2 .and. ansats(nl21,6,0)<=2)) &
                                       cycle
                                    do plusl7 = min(anl7,16), max(anl7 - 14,0)&
                                       , -1
                                    ansats(nl21,7,1) = plusl7
                                    ansats(nl21,7,0) = anl7 - plusl7
!     21l
                                    call sluggo (nl21, 8, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,8), low, start(nl21&
                                       ,8), stopp(nl21,8))
                                    do anl8 = start(nl21,8), stopp(nl21,8), -1
                                    antel(nl21,8) = anl8 + antel(nl21,7)
                                    if (.not.(antel(nl21,8)<=antal .and. ansats(&
                                       nl21,7,1)<=2 .and. ansats(nl21,7,0)<=2)) &
                                       cycle
                                    do plusl8 = min(anl8,18), max(anl8 - 16,0)&
                                       , -1
                                    ansats(nl21,8,1) = plusl8
                                    ansats(nl21,8,0) = anl8 - plusl8
!     21m
                                    call sluggo (nl21, 9, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,9), low, start(nl21&
                                       ,9), stopp(nl21,9))
                                    do anl9 = start(nl21,9), stopp(nl21,9), -1
                                    antel(nl21,9) = anl9 + antel(nl21,8)
                                    if (.not.(antel(nl21,9)<=antal .and. ansats(&
                                       nl21,8,1)<=2 .and. ansats(nl21,8,0)<=2)) &
                                       cycle
                                    do plusl9 = min(anl9,20), max(anl9 - 18,0)&
                                       , -1
                                    ansats(nl21,9,1) = plusl9
                                    ansats(nl21,9,0) = anl9 - plusl9
!     21n
                                    call sluggo (nl21, 10, varmax, varupp, varned&
                                       , ansats, org, lock(nl21,10), low, start(&
                                       nl21,10), stopp(nl21,10))
                                    do anla = start(nl21,10), stopp(nl21,10), -1
                                    antel(nl21,10) = anla + antel(nl21,9)
                                    !if (.not.(antel(nl21,10)==antal .and. ansats(&
                                    if (.not.(antel(nl21,10)<=antal .and. ansats(&
                                       nl21,9,1)<=2 .and. ansats(nl21,9,0)<=2)) &
                                       cycle
                                    do plusla = min(anla,22), max(anla - 20,0)&
                                       , -1
...
! n = 22
                                    nm22 = 22
!     22s
                                    call sluggo (nm22, 0, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,0), low, start(nm22&
                                       ,0), stopp(nm22,0))
                                    do anm0 = start(nm22,0), stopp(nm22,0), -1
...
!     22p
                                    call sluggo (nm22, 1, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,1), low, start(nm22&
                                       ,1), stopp(nm22,1))
                                    do anm1 = start(nm22,1), stopp(nm22,1), -1
                                    antel(nm22,1) = anm1 + antel(nm22,0)
                                    if (antel(nm22,1) > antal) cycle
                                    do plusm1 = min(anm1,4), max(anm1 - 2,0), &
                                       -1
                                    ansats(nm22,1,1) = plusm1
                                    ansats(nm22,1,0) = anm1 - plusm1
!     22d
                                    call sluggo (nm22, 2, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,2), low, start(nm22&
                                       ,2), stopp(nm22,2))
                                    do anm2 = start(nm22,2), stopp(nm22,2), -1
                                    antel(nm22,2) = anm2 + antel(nm22,1)
                                    if (antel(nm22,2) > antal) cycle
                                    do plusm2 = min(anm2,6), max(anm2 - 4,0), &
                                       -1
                                    ansats(nm22,2,1) = plusm2
                                    ansats(nm22,2,0) = anm2 - plusm2
!     22f
                                    call sluggo (nm22, 3, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,3), low, start(nm22&
                                       ,3), stopp(nm22,3))
                                    do anm3 = start(nm22,3), stopp(nm22,3), -1
                                    antel(nm22,3) = anm3 + antel(nm22,2)
                                    if (antel(nm22,3) > antal) cycle
                                    do plusm3 = min(anm3,8), max(anm3 - 6,0), &
                                       -1
                                    ansats(nm22,3,1) = plusm3
                                    ansats(nm22,3,0) = anm3 - plusm3
!     22g
                                    call sluggo (nm22, 4, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,4), low, start(nm22&
                                       ,4), stopp(nm22,4))
                                    do anm4 = start(nm22,4), stopp(nm22,4), -1
                                    antel(nm22,4) = anm4 + antel(nm22,3)
                                    if (antel(nm22,4) > antal) cycle
                                    do plusm4 = min(anm4,10), max(anm4 - 8,0), &
                                       -1
                                    ansats(nm22,4,1) = plusm4
                                    ansats(nm22,4,0) = anm4 - plusm4
!     22h
                                    call sluggo (nm22, 5, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,5), low, start(nm22&
                                       ,5), stopp(nm22,5))
                                    do anm5 = start(nm22,5), stopp(nm22,5), -1
                                    antel(nm22,5) = anm5 + antel(nm22,4)
                                    if (antel(nm22,5)>antal .or. ansats(nm22,4,1)>2&
                                       ) cycle
                                    do plusm5 = min(anm5,12), max(anm5 - 10,0)&
                                       , -1
                                    ansats(nm22,5,1) = plusm5
                                    ansats(nm22,5,0) = anm5 - plusm5
!     22i
                                    call sluggo (nm22, 6, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,6), low, start(nm22&
                                       ,6), stopp(nm22,6))
                                    do anm6 = start(nm22,6), stopp(nm22,6), -1
                                    antel(nm22,6) = anm6 + antel(nm22,5)
                                    if (.not.(antel(nm22,6)<=antal .and. ansats(&
                                       nm22,5,1)<=2 .and. ansats(nm22,5,0)<=2)) &
                                       cycle
                                    do plusm6 = min(anm6,14), max(anm6 - 12,0)&
                                       , -1
                                    ansats(nm22,6,1) = plusm6
                                    ansats(nm22,6,0) = anm6 - plusm6
!     22k
                                    call sluggo (nm22, 7, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,7), low, start(nm22&
                                       ,7), stopp(nm22,7))
                                    do anm7 = start(nm22,7), stopp(nm22,7), -1
                                    antel(nm22,7) = anm7 + antel(nm22,6)
                                    if (.not.(antel(nm22,7)<=antal .and. ansats(&
                                       nm22,6,1)<=2 .and. ansats(nm22,6,0)<=2)) &
                                       cycle
                                    do plusm7 = min(anm7,16), max(anm7 - 14,0)&
                                       , -1
                                    ansats(nm22,7,1) = plusm7
                                    ansats(nm22,7,0) = anm7 - plusm7
!     22l
                                    call sluggo (nm22, 8, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,8), low, start(nm22&
                                       ,8), stopp(nm22,8))
                                    do anm8 = start(nm22,8), stopp(nm22,8), -1
                                    antel(nm22,8) = anm8 + antel(nm22,7)
                                    if (.not.(antel(nm22,8)<=antal .and. ansats(&
                                       nm22,7,1)<=2 .and. ansats(nm22,7,0)<=2)) &
                                       cycle
                                    do plusm8 = min(anm8,18), max(anm8 - 16,0)&
                                       , -1
                                    ansats(nm22,8,1) = plusm8
                                    ansats(nm22,8,0) = anm8 - plusm8
!     22m
                                    call sluggo (nm22, 9, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,9), low, start(nm22&
                                       ,9), stopp(nm22,9))
                                    do anm9 = start(nm22,9), stopp(nm22,9), -1
                                    antel(nm22,9) = anm9 + antel(nm22,8)
                                    if (.not.(antel(nm22,9)<=antal .and. ansats(&
                                       nm22,8,1)<=2 .and. ansats(nm22,8,0)<=2)) &
                                       cycle
                                    do plusm9 = min(anm9,20), max(anm9 - 18,0)&
                                       , -1
                                    ansats(nm22,9,1) = plusm9
                                    ansats(nm22,9,0) = anm9 - plusm9
!     22n
                                    call sluggo (nm22, 10, varmax, varupp, varned&
                                       , ansats, org, lock(nm22,10), low, start(&
                                       nm22,10), stopp(nm22,10))
                                    do anma = start(nm22,10), stopp(nm22,10), -1
                                    antel(nm22,10) = anma + antel(nm22,9)
                                    !if (.not.(antel(nm22,10)==antal .and. ansats(&
                                    if (.not.(antel(nm22,10)<=antal .and. ansats(&
                                       nm22,9,1)<=2 .and. ansats(nm22,9,0)<=2)) &
                                       cycle
                                    do plusma = min(anma,22), max(anma - 20,0)&
                                       , -1
...
! n = 23
                                    nn23 = 23
!     23s
                                    call sluggo (nn23, 0, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,0), low, start(nn23&
                                       ,0), stopp(nn23,0))
                                    do ann0 = start(nn23,0), stopp(nn23,0), -1
...
!     23p
                                    call sluggo (nn23, 1, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,1), low, start(nn23&
                                       ,1), stopp(nn23,1))
                                    do ann1 = start(nn23,1), stopp(nn23,1), -1
                                    antel(nn23,1) = ann1 + antel(nn23,0)
                                    if (antel(nn23,1) > antal) cycle
                                    do plusn1 = min(ann1,4), max(ann1 - 2,0), &
                                       -1
                                    ansats(nn23,1,1) = plusn1
                                    ansats(nn23,1,0) = ann1 - plusn1
!     23d
                                    call sluggo (nn23, 2, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,2), low, start(nn23&
                                       ,2), stopp(nn23,2))
                                    do ann2 = start(nn23,2), stopp(nn23,2), -1
                                    antel(nn23,2) = ann2 + antel(nn23,1)
                                    if (antel(nn23,2) > antal) cycle
                                    do plusn2 = min(ann2,6), max(ann2 - 4,0), &
                                       -1
                                    ansats(nn23,2,1) = plusn2
                                    ansats(nn23,2,0) = ann2 - plusn2
!     23f
                                    call sluggo (nn23, 3, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,3), low, start(nn23&
                                       ,3), stopp(nn23,3))
                                    do ann3 = start(nn23,3), stopp(nn23,3), -1
                                    antel(nn23,3) = ann3 + antel(nn23,2)
                                    if (antel(nn23,3) > antal) cycle
                                    do plusn3 = min(ann3,8), max(ann3 - 6,0), &
                                       -1
                                    ansats(nn23,3,1) = plusn3
                                    ansats(nn23,3,0) = ann3 - plusn3
!     23g
                                    call sluggo (nn23, 4, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,4), low, start(nn23&
                                       ,4), stopp(nn23,4))
                                    do ann4 = start(nn23,4), stopp(nn23,4), -1
                                    antel(nn23,4) = ann4 + antel(nn23,3)
                                    if (antel(nn23,4) > antal) cycle
                                    do plusn4 = min(ann4,10), max(ann4 - 8,0), &
                                       -1
                                    ansats(nn23,4,1) = plusn4
                                    ansats(nn23,4,0) = ann4 - plusn4
!     23h
                                    call sluggo (nn23, 5, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,5), low, start(nn23&
                                       ,5), stopp(nn23,5))
                                    do ann5 = start(nn23,5), stopp(nn23,5), -1
                                    antel(nn23,5) = ann5 + antel(nn23,4)
                                    if (antel(nn23,5)>antal .or. ansats(nn23,4,1)>2&
                                       ) cycle
                                    do plusn5 = min(ann5,12), max(ann5 - 10,0)&
                                       , -1
                                    ansats(nn23,5,1) = plusn5
                                    ansats(nn23,5,0) = ann5 - plusn5
!     23i
                                    call sluggo (nn23, 6, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,6), low, start(nn23&
                                       ,6), stopp(nn23,6))
                                    do ann6 = start(nn23,6), stopp(nn23,6), -1
                                    antel(nn23,6) = ann6 + antel(nn23,5)
                                    if (.not.(antel(nn23,6)<=antal .and. ansats(&
                                       nn23,5,1)<=2 .and. ansats(nn23,5,0)<=2)) &
                                       cycle
                                    do plusn6 = min(ann6,14), max(ann6 - 12,0)&
                                       , -1
                                    ansats(nn23,6,1) = plusn6
                                    ansats(nn23,6,0) = ann6 - plusn6
!     23k
                                    call sluggo (nn23, 7, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,7), low, start(nn23&
                                       ,7), stopp(nn23,7))
                                    do ann7 = start(nn23,7), stopp(nn23,7), -1
                                    antel(nn23,7) = ann7 + antel(nn23,6)
                                    if (.not.(antel(nn23,7)<=antal .and. ansats(&
                                       nn23,6,1)<=2 .and. ansats(nn23,6,0)<=2)) &
                                       cycle
                                    do plusn7 = min(ann7,16), max(ann7 - 14,0)&
                                       , -1
                                    ansats(nn23,7,1) = plusn7
                                    ansats(nn23,7,0) = ann7 - plusn7
!     23l
                                    call sluggo (nn23, 8, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,8), low, start(nn23&
                                       ,8), stopp(nn23,8))
                                    do ann8 = start(nn23,8), stopp(nn23,8), -1
                                    antel(nn23,8) = ann8 + antel(nn23,7)
                                    if (.not.(antel(nn23,8)<=antal .and. ansats(&
                                       nn23,7,1)<=2 .and. ansats(nn23,7,0)<=2)) &
                                       cycle
                                    do plusn8 = min(ann8,18), max(ann8 - 16,0)&
                                       , -1
                                    ansats(nn23,8,1) = plusn8
                                    ansats(nn23,8,0) = ann8 - plusn8
!     23m
                                    call sluggo (nn23, 9, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,9), low, start(nn23&
                                       ,9), stopp(nn23,9))
                                    do ann9 = start(nn23,9), stopp(nn23,9), -1
                                    antel(nn23,9) = ann9 + antel(nn23,8)
                                    if (.not.(antel(nn23,9)<=antal .and. ansats(&
                                       nn23,8,1)<=2 .and. ansats(nn23,8,0)<=2)) &
                                       cycle
                                    do plusn9 = min(ann9,20), max(ann9 - 18,0)&
                                       , -1
                                    ansats(nn23,9,1) = plusn9
                                    ansats(nn23,9,0) = ann9 - plusn9
!     23n
                                    call sluggo (nn23, 10, varmax, varupp, varned&
                                       , ansats, org, lock(nn23,10), low, start(&
                                       nn23,10), stopp(nn23,10))
                                    do anna = start(nn23,10), stopp(nn23,10), -1
                                    antel(nn23,10) = anna + antel(nn23,9)
                                    !if (.not.(antel(nn23,10)==antal .and. ansats(&
                                    if (.not.(antel(nn23,10)<=antal .and. ansats(&
                                       nn23,9,1)<=2 .and. ansats(nn23,9,0)<=2)) &
                                       cycle
                                    do plusna = min(anna,22), max(anna - 20,0)&
                                       , -1
...
! n = 24
                                    no24 = 24
!     24s
                                    call sluggo (no24, 0, varmax, varupp, varned&
                                       , ansats, org, lock(no24,0), low, start(no24&
                                       ,0), stopp(no24,0))
                                    do ano0 = start(no24,0), stopp(no24,0), -1
...
!     24p
                                    call sluggo (no24, 1, varmax, varupp, varned&
                                       , ansats, org, lock(no24,1), low, start(no24&
                                       ,1), stopp(no24,1))
                                    do ano1 = start(no24,1), stopp(no24,1), -1
                                    antel(no24,1) = ano1 + antel(no24,0)
                                    if (antel(no24,1) > antal) cycle
                                    do pluso1 = min(ano1,4), max(ano1 - 2,0), &
                                       -1
                                    ansats(no24,1,1) = pluso1
                                    ansats(no24,1,0) = ano1 - pluso1
!     24d
                                    call sluggo (no24, 2, varmax, varupp, varned&
                                       , ansats, org, lock(no24,2), low, start(no24&
                                       ,2), stopp(no24,2))
                                    do ano2 = start(no24,2), stopp(no24,2), -1
                                    antel(no24,2) = ano2 + antel(no24,1)
                                    if (antel(no24,2) > antal) cycle
                                    do pluso2 = min(ano2,6), max(ano2 - 4,0), &
                                       -1
                                    ansats(no24,2,1) = pluso2
                                    ansats(no24,2,0) = ano2 - pluso2
!     24f
                                    call sluggo (no24, 3, varmax, varupp, varned&
                                       , ansats, org, lock(no24,3), low, start(no24&
                                       ,3), stopp(no24,3))
                                    do ano3 = start(no24,3), stopp(no24,3), -1
                                    antel(no24,3) = ano3 + antel(no24,2)
                                    if (antel(no24,3) > antal) cycle
                                    do pluso3 = min(ano3,8), max(ano3 - 6,0), &
                                       -1
                                    ansats(no24,3,1) = pluso3
                                    ansats(no24,3,0) = ano3 - pluso3
!     24g
                                    call sluggo (no24, 4, varmax, varupp, varned&
                                       , ansats, org, lock(no24,4), low, start(no24&
                                       ,4), stopp(no24,4))
                                    do ano4 = start(no24,4), stopp(no24,4), -1
                                    antel(no24,4) = ano4 + antel(no24,3)
                                    if (antel(no24,4) > antal) cycle
                                    do pluso4 = min(ano4,10), max(ano4 - 8,0), &
                                       -1
                                    ansats(no24,4,1) = pluso4
                                    ansats(no24,4,0) = ano4 - pluso4
!     24h
                                    call sluggo (no24, 5, varmax, varupp, varned&
                                       , ansats, org, lock(no24,5), low, start(no24&
                                       ,5), stopp(no24,5))
                                    do ano5 = start(no24,5), stopp(no24,5), -1
                                    antel(no24,5) = ano5 + antel(no24,4)
                                    if (antel(no24,5)>antal .or. ansats(no24,4,1)>2&
                                       ) cycle
                                    do pluso5 = min(ano5,12), max(ano5 - 10,0)&
                                       , -1
                                    ansats(no24,5,1) = pluso5
                                    ansats(no24,5,0) = ano5 - pluso5
!     24i
                                    call sluggo (no24, 6, varmax, varupp, varned&
                                       , ansats, org, lock(no24,6), low, start(no24&
                                       ,6), stopp(no24,6))
                                    do ano6 = start(no24,6), stopp(no24,6), -1
                                    antel(no24,6) = ano6 + antel(no24,5)
                                    if (.not.(antel(no24,6)<=antal .and. ansats(&
                                       no24,5,1)<=2 .and. ansats(no24,5,0)<=2)) &
                                       cycle
                                    do pluso6 = min(ano6,14), max(ano6 - 12,0)&
                                       , -1
                                    ansats(no24,6,1) = pluso6
                                    ansats(no24,6,0) = ano6 - pluso6
!     24k
                                    call sluggo (no24, 7, varmax, varupp, varned&
                                       , ansats, org, lock(no24,7), low, start(no24&
                                       ,7), stopp(no24,7))
                                    do ano7 = start(no24,7), stopp(no24,7), -1
                                    antel(no24,7) = ano7 + antel(no24,6)
                                    if (.not.(antel(no24,7)<=antal .and. ansats(&
                                       no24,6,1)<=2 .and. ansats(no24,6,0)<=2)) &
                                       cycle
                                    do pluso7 = min(ano7,16), max(ano7 - 14,0)&
                                       , -1
                                    ansats(no24,7,1) = pluso7
                                    ansats(no24,7,0) = ano7 - pluso7
!     24l
                                    call sluggo (no24, 8, varmax, varupp, varned&
                                       , ansats, org, lock(no24,8), low, start(no24&
                                       ,8), stopp(no24,8))
                                    do ano8 = start(no24,8), stopp(no24,8), -1
                                    antel(no24,8) = ano8 + antel(no24,7)
                                    if (.not.(antel(no24,8)<=antal .and. ansats(&
                                       no24,7,1)<=2 .and. ansats(no24,7,0)<=2)) &
                                       cycle
                                    do pluso8 = min(ano8,18), max(ano8 - 16,0)&
                                       , -1
                                    ansats(no24,8,1) = pluso8
                                    ansats(no24,8,0) = ano8 - pluso8
!     24m
                                    call sluggo (no24, 9, varmax, varupp, varned&
                                       , ansats, org, lock(no24,9), low, start(no24&
                                       ,9), stopp(no24,9))
                                    do ano9 = start(no24,9), stopp(no24,9), -1
                                    antel(no24,9) = ano9 + antel(no24,8)
                                    if (.not.(antel(no24,9)<=antal .and. ansats(&
                                       no24,8,1)<=2 .and. ansats(no24,8,0)<=2)) &
                                       cycle
                                    do pluso9 = min(ano9,20), max(ano9 - 18,0)&
                                       , -1
                                    ansats(no24,9,1) = pluso9
                                    ansats(no24,9,0) = ano9 - pluso9
!     24n
                                    call sluggo (no24, 10, varmax, varupp, varned&
                                       , ansats, org, lock(no24,10), low, start(&
                                       no24,10), stopp(no24,10))
                                    do anoa = start(no24,10), stopp(no24,10), -1
                                    antel(no24,10) = anoa + antel(no24,9)
                                    !if (.not.(antel(no24,10)==antal .and. ansats(&
                                    if (.not.(antel(no24,10)<=antal .and. ansats(&
                                       no24,9,1)<=2 .and. ansats(no24,9,0)<=2)) &
                                       cycle
                                    do plusoa = min(anoa,22), max(anoa - 20,0)&
                                       , -1
...
! n = 25
                                    np25 = 25
!     25s
                                    call sluggo (np25, 0, varmax, varupp, varned&
                                       , ansats, org, lock(np25,0), low, start(np25&
                                       ,0), stopp(np25,0))
                                    do anp0 = start(np25,0), stopp(np25,0), -1
...
!     25p
                                    call sluggo (np25, 1, varmax, varupp, varned&
                                       , ansats, org, lock(np25,1), low, start(np25&
                                       ,1), stopp(np25,1))
                                    do anp1 = start(np25,1), stopp(np25,1), -1
                                    antel(np25,1) = anp1 + antel(np25,0)
                                    if (antel(np25,1) > antal) cycle
                                    do plusp1 = min(anp1,4), max(anp1 - 2,0), &
                                       -1
                                    ansats(np25,1,1) = plusp1
                                    ansats(np25,1,0) = anp1 - plusp1
!     25d
                                    call sluggo (np25, 2, varmax, varupp, varned&
                                       , ansats, org, lock(np25,2), low, start(np25&
                                       ,2), stopp(np25,2))
                                    do anp2 = start(np25,2), stopp(np25,2), -1
                                    antel(np25,2) = anp2 + antel(np25,1)
                                    if (antel(np25,2) > antal) cycle
                                    do plusp2 = min(anp2,6), max(anp2 - 4,0), &
                                       -1
                                    ansats(np25,2,1) = plusp2
                                    ansats(np25,2,0) = anp2 - plusp2
!     25f
                                    call sluggo (np25, 3, varmax, varupp, varned&
                                       , ansats, org, lock(np25,3), low, start(np25&
                                       ,3), stopp(np25,3))
                                    do anp3 = start(np25,3), stopp(np25,3), -1
                                    antel(np25,3) = anp3 + antel(np25,2)
                                    if (antel(np25,3) > antal) cycle
                                    do plusp3 = min(anp3,8), max(anp3 - 6,0), &
                                       -1
                                    ansats(np25,3,1) = plusp3
                                    ansats(np25,3,0) = anp3 - plusp3
!     25g
                                    call sluggo (np25, 4, varmax, varupp, varned&
                                       , ansats, org, lock(np25,4), low, start(np25&
                                       ,4), stopp(np25,4))
                                    do anp4 = start(np25,4), stopp(np25,4), -1
                                    antel(np25,4) = anp4 + antel(np25,3)
                                    if (antel(np25,4) > antal) cycle
                                    do plusp4 = min(anp4,10), max(anp4 - 8,0), &
                                       -1
                                    ansats(np25,4,1) = plusp4
                                    ansats(np25,4,0) = anp4 - plusp4
!     25h
                                    call sluggo (np25, 5, varmax, varupp, varned&
                                       , ansats, org, lock(np25,5), low, start(np25&
                                       ,5), stopp(np25,5))
                                    do anp5 = start(np25,5), stopp(np25,5), -1
                                    antel(np25,5) = anp5 + antel(np25,4)
                                    if (antel(np25,5)>antal .or. ansats(np25,4,1)>2&
                                       ) cycle
                                    do plusp5 = min(anp5,12), max(anp5 - 10,0)&
                                       , -1
                                    ansats(np25,5,1) = plusp5
                                    ansats(np25,5,0) = anp5 - plusp5
!     25i
                                    call sluggo (np25, 6, varmax, varupp, varned&
                                       , ansats, org, lock(np25,6), low, start(np25&
                                       ,6), stopp(np25,6))
                                    do anp6 = start(np25,6), stopp(np25,6), -1
                                    antel(np25,6) = anp6 + antel(np25,5)
                                    if (.not.(antel(np25,6)<=antal .and. ansats(&
                                       np25,5,1)<=2 .and. ansats(np25,5,0)<=2)) &
                                       cycle
                                    do plusp6 = min(anp6,14), max(anp6 - 12,0)&
                                       , -1
                                    ansats(np25,6,1) = plusp6
                                    ansats(np25,6,0) = anp6 - plusp6
!     25k
                                    call sluggo (np25, 7, varmax, varupp, varned&
                                       , ansats, org, lock(np25,7), low, start(np25&
                                       ,7), stopp(np25,7))
                                    do anp7 = start(np25,7), stopp(np25,7), -1
                                    antel(np25,7) = anp7 + antel(np25,6)
                                    if (.not.(antel(np25,7)<=antal .and. ansats(&
                                       np25,6,1)<=2 .and. ansats(np25,6,0)<=2)) &
                                       cycle
                                    do plusp7 = min(anp7,16), max(anp7 - 14,0)&
                                       , -1
                                    ansats(np25,7,1) = plusp7
                                    ansats(np25,7,0) = anp7 - plusp7
!     25l
                                    call sluggo (np25, 8, varmax, varupp, varned&
                                       , ansats, org, lock(np25,8), low, start(np25&
                                       ,8), stopp(np25,8))
                                    do anp8 = start(np25,8), stopp(np25,8), -1
                                    antel(np25,8) = anp8 + antel(np25,7)
                                    if (.not.(antel(np25,8)<=antal .and. ansats(&
                                       np25,7,1)<=2 .and. ansats(np25,7,0)<=2)) &
                                       cycle
                                    do plusp8 = min(anp8,18), max(anp8 - 16,0)&
                                       , -1
                                    ansats(np25,8,1) = plusp8
                                    ansats(np25,8,0) = anp8 - plusp8
!     25m
                                    call sluggo (np25, 9, varmax, varupp, varned&
                                       , ansats, org, lock(np25,9), low, start(np25&
                                       ,9), stopp(np25,9))
                                    do anp9 = start(np25,9), stopp(np25,9), -1
                                    antel(np25,9) = anp9 + antel(np25,8)
                                    if (.not.(antel(np25,9)<=antal .and. ansats(&
                                       np25,8,1)<=2 .and. ansats(np25,8,0)<=2)) &
                                       cycle
                                    do plusp9 = min(anp9,20), max(anp9 - 18,0)&
                                       , -1
                                    ansats(np25,9,1) = plusp9
                                    ansats(np25,9,0) = anp9 - plusp9
!     25n
                                    call sluggo (np25, 10, varmax, varupp, varned&
                                       , ansats, org, lock(np25,10), low, start(&
                                       np25,10), stopp(np25,10))
                                    do anpa = start(np25,10), stopp(np25,10), -1
                                    antel(np25,10) = anpa + antel(np25,9)
                                    if (.not.(antel(np25,10)==antal .and. ansats(&
                                    !if (.not.(antel(np25,10)<=antal .and. ansats(&
                                       np25,9,1)<=2 .and. ansats(np25,9,0)<=2)) &
                                       cycle
                                    do pluspa = min(anpa,22), max(anpa - 20,0)&
                                       , -1
