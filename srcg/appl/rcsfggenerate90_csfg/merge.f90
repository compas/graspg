...
!-----------------------------------------------
      integer , dimension(25,0:10,0:1) :: pop1, pop2
      integer :: skal1, skal2, i, j, k, cf, stopp1, stopp2
      integer , dimension(25,0:10,0:1) :: popo
      integer :: ii1
...
!-----------------------------------------------
      ...
      ii1 = mod(ii,2)
      if (ii1 == 0) then
         open(unit=utfil, file='rcsfg.out', &
              status='unknown', position='asis')
      else
         open(unit=utfil, file='fil1.dat', &
              status='unknown', position='asis')
      endif
      open(unit=nyfil, file='clist.new', &
           status='unknown', position='asis')
      slut1 = .FALSE.
      slut2 = single
      cf = 0
   ...
   80    continue
      ...
      close(fil_2)
      close(utfil)
! CYC 2024/08/     close(nyfil)
      close(unit=nyfil,status='delete')
