...
!      CHARACTER(LEN=500) :: RECORD, RECORD2
      CHARACTER(LEN=2000) :: RECORD, RECORD2
! CYC
      !CHARACTER(LEN=2), DIMENSION(27) :: SYMLST
      INTEGER, PARAMETER  :: NSYMLST = 27
      CHARACTER(LEN=2), DIMENSION(NSYMLST) :: SYMLST
!
...
!              READ (RECI,'(1A1)',IOSTAT = IOS) SYM(1:1)
               SYM(1:1) = RECI
               IF (IOS /= 0) THEN
                  WRITE (ISTDE, *) 'PRSRSL: Symmetry ', RECI,  &
                    ' could not be decoded.'
                  STOP
               ENDIF
               IEND = IEND - 1
