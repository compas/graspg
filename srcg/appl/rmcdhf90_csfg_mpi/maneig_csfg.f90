!***********************************************************************
!                                                                      *
      SUBROUTINE MANEIG_CSFG(DVDFIRST, JBLOCK, &
...
! CYC: Restart mode:
      INTEGER :: NMV0, IRESTART_GDVD, ISTEP, IITMP, NSTEP
      INTEGER :: IOLPCK
...
!      LIM = MIN(NCF,2*NVECT + 20)
      ...
      NIV = NVEX
!CYC      MAXITR = MIN(NVECT*200,NCF)
      MAXITR = MAX(NVECT*1000,NCF)
!      N1000 = 2000
      IOLPCK = 1000
      ncsfDF1 = scfDF1(JBLOCK)
      N1000 = MIN(ncsfDF1,IOLPCK)
!
...
!         IF (MYID == 0) WRITE(*,*)"Exiting from INIESTmpi ..."
      else
...
! is no need to call dvdson if block size <= n1000
      ...
      IF (NCF > N1000) THEN
         IF (myid == 0) WRITE (*,*) &
          'Calling dvdson!!!, ncsf, nvect, jblock=',ncf, nvect,jblock
...
!         call system_clock (ncount1, ncount_rate, ncount_max)
         ...
         CALL GDVD(SPICMVsym,NCF,LIM,DIAG,ILOW,IHIGH,JWORK,NIV,MBLOCK,  &
...
!         if (myid.eq.0) write(*,*) "This block diagonalization (s): ",nsecs
!         if (myid.eq.0) write(*,*)
      ENDIF
!
...
!===============================================================
  ...
  301 FORMAT(' DVDSON: ',1I5,' loops; ',1I5,' matrix-vector multiplies.')
