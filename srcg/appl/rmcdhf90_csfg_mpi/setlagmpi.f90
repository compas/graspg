...
!-----------------------------------------------------------------------
      ...
      IF (FIRST) THEN
!
!   Not to skip the potential calculations for varied orbitals    
!
        DO I = 1, NW
          LSKIPPOT(I) = .TRUE.
          IF (.NOT. LFIX(I)) LSKIPPOT(I) = .FALSE.
        ENDDO
!=======================================================================
...
!         ENDIF
         ...
         ELSE
            ...
            !*** Decode index ***
               ...
               L1 = IECCLI/KEY
               L2 = IECCLI - KEY*L1
               IF (MYID == 0) WRITE (*, 305) NP(L2), NH(L2), NP(L1), NH(L1)
!
!   Not to skip the potential calculations for orbitals involved within
!   any Lagrange multipliers.   
!
               LSKIPPOT(L1) = .FALSE.
               LSKIPPOT(L2) = .FALSE.
            END DO
         ...
         FIRST = .FALSE.
!   CYC  May 2022
!   On the first call, just set NEC, IECC, and LSKIPPOT, RETURN

!CYC 2024/03/23
!         write(*,*)'myid =', myid, LSKIPPOT(1:NW) 
         RETURN  
      ENDIF
...
!start dbg
!           WRITE (81,*)'2, RESULT, RIJM, UCFM, ECV'
!           WRITE (81,*)RESULT, RIJM, UCFJ, ECV(LI)
!end dbg
...
!               WRITE (81,'(3(1pe22.14))')RESULT, OBQDIF, ECV(LI)
               !DO i = 1, MTP
...
!start dbg
!               WRITE (81,*)'4, RESULT, RIJM, OBQSUM, ECV, TA'
!               WRITE (81,*)RESULT, RIJM, OBQSUM, ECV(LI)
               !DO i = 1, MTP
               !   WRITE (81,*) i, TA(i), r(i), rp(i)
               !ENDDO
!end dbg
