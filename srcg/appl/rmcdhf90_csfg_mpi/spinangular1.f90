!***********************************************************************
!                                                                      *
      SUBROUTINE SPINANGULAR1 (JASAV, JBSAV, IUNITF, NPOS, JCSFASAV, JCSFBSAV)
!                                                                      *
...
! There is no T-contributions from diagonal matrixelement of labeling
! CSFs.
      IF (.NOT. LCHM) GOTO 200

! The spin-angular coefficients for one-body contributions for diagonal
! matrixelements are the occupy numbers.
!
...
! potentials involving the T-coefficients. 
      DO IA = 1, NW
        ...
        IF (IQTMP .GT. 0) THEN
          ...
          EMTBLOCK(1,1) = EMTBLOCK(1,1) + IQTMP * TEGRAL 
        ENDIF
      ENDDO
!
...
200   CONTINUE 
      ...
      !IF (JASAV == 1) THEN
      !  WRITE(*,*)'JASAV, NVCOEF =', JASAV, NVCOEF
      !  WRITE(*, '(I8, 2X,4HICW=,I8, 2X,4HIRW=,I8, &
      !                 2X,5HTYPE=,I4, 2X,6HMTYPE=,I4, 2X,3HNV=,I4)')&
      !                 NRKCO, JASAV, JBSAV, IUNITF-2000, 1, NVCOEF
      !  write(*,'(4x,7hIVCOEF=,1000i11)')(J, J=1,NVCOEF)
      !  write(*,'(4x,7hLabel1=,1000i11)')(LABEL(1,J), J=1,NVCOEF)
      !  write(*,'(4x,7hLabel2=,1000i11)')(LABEL(2,J), J=1,NVCOEF)
      !  write(*,'(4x,7hLabel3=,1000i11)')(LABEL(3,J), J=1,NVCOEF)
      !  write(*,'(4x,7hLabel4=,1000i11)')(LABEL(4,J), J=1,NVCOEF)
      !  write(*,'(4x,7hLabel5=,1000i11)')(LABEL(5,J), J=1,NVCOEF)
      !  write(*,'(4x,7hCoeff =,1000(1pe11.3))')(Coeff(J), J=1,NVCOEF)
      !ENDIF
      DO 2 IV = 1, NVCOEF      
        VCOEFF = COEFF(IV)
    ...
    2 CONTINUE
      ...
      IF (LCHM) CALL TRANSFER_CSFG(JASAV+NCFGPAST, JBSAV+NCFGPAST)

      RETURN
      END SUBROUTINE SPINANGULAR1
