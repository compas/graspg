!***********************************************************************
!                                                                      *
      SUBROUTINE SPINANGULAR2  (JASAV, JBSAV, IUNITF, NPOS, JCSFASAV, JCSFBSAV)
!                                                                      *
...
!
      IF (NORBCOL >= 2) THEN
        ...
        IA = 0
        CALL read_TV(1, JASAV, JBSAV, IUNITF, 2, IA, IB, TCOEFF)
        IF (ABS(TCOEFF-1.D0) > 1.0D-8) &
          STOP "ABS(TCOEFF-1.D0) > 1.0D-8 in spinangular2.f90 ..."
        !IF (LPRINT.AND.IA.NE.0.AND.ABS(TSHELL(1)).GT.CUTOFF) &
        !  CALL PRINTLABELV(1, IC, IR, 2, 2, IA, IB, TCOEFF)
          ...
          DO I = 1, NORBROW - 1
            ...
            DO J = I + 1, NORBCOL 
              ...
              IF (LABTVFRST.OR.LCPOT) THEN
                CALL SETCOF_NDA_CSFG(I, J, IA, IB, TCOEFF)
              ELSE
                CALL IABINT(IA, IB, TEGRAL)
                EMTBLOCK(I,J) = TCOEFF * TEGRAL 
              ENDIF
            ENDDO
          ENDDO
        ENDIF
...
! determied by IPSym(1), IPSym(2)
        ...
        ELSEIF (NSYMCR .EQ. 3) THEN
          WRITE(*,'(a3, i7, 2x, a3, i7, 2x, a5, 5i3)')              &
               'IC=', JASAV, 'IR=', JBSAV, 'LABV=', LABV(1:5)
          STOP 'Warning!!! NSYMCR ERROR ***3*** 2'

        ELSEIF (NSYMCR .EQ. 4) THEN
          WRITE(*,'(a3, i7, 2x, a3, i7, 2x, a5, 5i3)')              &
               'IC=', JASAV, 'IR=', JBSAV, 'LABV=', LABV(1:5)
          STOP 'Warning!!! NSYMCR ERROR ***4*** 2'
        ENDIF
    2 CONTINUE
!
