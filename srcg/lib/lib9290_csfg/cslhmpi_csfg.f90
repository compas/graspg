...
!     node-0 does exactly the same as the serial code does
      IF (myid .EQ. 0) THEN
        CALL ALLOC   (ncfblk, nblkin+1, 'ncfblk', 'cslhmpi_csfg')
        CALL SETCSLL_CSFG (21, name, nblkin, nblock, ncfblk, ncftot, &
                       idblk)
        CALL RALLOC  (ncfblk, nblock+1, 'ncfblk', 'cslhmpi_csfg')
         REWIND (21)
         READ   (21,*)
         !..Load header of <name> file
         CALL LODCSH_CSFG (21, NCORE)
      ENDIF
