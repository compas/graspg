...
!
      IF (JB /= LOADALL) THEN
         WRITE (6, *) 'Loading CSF(G) File for block ', JB
      ELSE
         WRITE (6, *) 'Loading CSF(G) File for ALL blocks '
      ENDIF
...
!
!CYC Aug 2024
!This section is too time-consuming for a large list 
!         IF (NCF > 1) THEN
!            DO J = 1, NCF - 1
!               DO I = NCORP1, NW
...
!      NELEC = NCOREL+NPEEL
      IF (NCOREL + NPEEL /= NELEC) THEN
         ...
         STOP
      ENDIF
      WRITE (6,*)'There are ',NCF,' relativistic CSF(G)s... load complete;'
      RETURN
!
   26 CONTINUE
      BACKSPACE (NFILE)
