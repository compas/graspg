!***********************************************************************
!
      SUBROUTINE LODSTATE_CSFG(IDBLK)
!
...
!
!
!  Modified for CSFG version by Chongyang Chen Oct 2024
!
!***********************************************************************
...
!PERJ      WRITE (ISTDE, *) 'There are ', NBLOCK, ' blocks  ', &
!         '(block   J/Parity   NCSF(G)s   NCFTr):'
!      WRITE (ISTDE, '( 4(I3, 1X, A5, I8, I8, 5X) )')      &
!         (J, IDBLK(J)(1:5), NCFBLK(J),TotCSFs_perblock(3,J), &
!         J=1, NBLOCK)
  ...
  123 CONTINUE
      DO JBLOCK = 1, NBLOCK
         NCF = NCFBLK(JBLOCK)
  234    CONTINUE
         WRITE (ISTDE, *) 'Block ', JBLOCK, '  NCSF(G)s =', NCF, &
            ' NCSFs =', TotCSFs_perblock(3,JBLOCK), &
            ' id = ', IDBLK(JBLOCK)(1:5)
...
!CYC         IF (NCMIN == 0) CALL DALLOC (ICCMIN, 'ICCMIN', 'LODSTATE' )
         IF (IERR < 0) GO TO 234
...
!CYC-2024
        IF (NBLOCK > 1) THEN        
          ...
          GO TO 123
        ELSE 
          WRITE(ISTDE, *)'No state is selected, just build H-matrix ...'
        ENDIF 
!CYC-2024 End
      ENDIF

      RETURN
      END SUBROUTINE LODSTATE_CSFG
