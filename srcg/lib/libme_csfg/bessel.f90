...
!
!      if (NP(IA) .le. NQEDMAX .and. NP(IB) .le. NQEDMAX ) then
! cychen: spectroscopic orbital using factor as input, generally being 0.0d0 and 1.0d0
         W = WFACT*DABS (E(IA)-E(IB))/C
!      else
! cychen: correlation orbital using difinitely factor of 0.0d0
!         W = 0.0d0*DABS (E(IA)-E(IB))/C
!      endif
