...
!-----------------------------------------------
      LOGICAL :: FOUND
      INTEGER :: key, loc, ju, jl, jm, newsiz, i
      INTEGER :: INDEX
!-----------------------------------------------
!
      KEY = NW + 1
...
!      INDEX = (((NU*KEY+IA)*KEY+IB)*KEY+IC)*KEY+ID
      INDEX = ID
      INDEX = INDEX + (((NU*KEY+IA)*KEY+IB)*KEY+IC)*KEY
!
      IF (.NOT. FIRST(6)) THEN
!
...
!
            ...
            ENDIF
! CYC
            INDTP6(LOC+2:NTPI(6)) = INDTP6(LOC+1:NTPI(6)-1)
            VALTP6(LOC+2:NTPI(6)) = VALTP6(LOC+1:NTPI(6)-1)
