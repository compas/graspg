...
!      REAL(DOUBLE), DIMENSION(1) :: UCF
      ...
      REAL(DOUBLE) :: ZEFF, RATIO, VALU, FZTMP
...
!CYC      IF (MYID.EQ.0)                                                 &
!CYC             WRITE(*,'(6A12)')                                       &
!CYC                 'n', 'kapa', 'NH', 'FZALF_ORIG', 'RATIO', 'SLFINT'
!
!
      DO J = 1, NW
!
...
! CYC 2023/07/11
            IF (ABS(RATIO) > 1.0D0) RATIO = 0.0D0
            FZTMP=FZALF (NPJ,KAPPA,ZEFF)
            VALU =  RATIO*FZTMP/DBLE (NPJ**3)
!            VALU = RATIO*FZALF(NPJ,KAPPA,ZEFF)/DBLE(NPJ**3)
            SLFINT(J) = VALU*ZEFF**4/(PI*C**3)
!
!CYC            IF (MYID.EQ.0)                                           &
!CYC             write(*,'(2I12,A12,3(1PE12.4))')                        &
!CYC                     NPJ,KAPPA,NH(J),FZTMP,RATIO,SLFINT(J)
         ELSE
!
