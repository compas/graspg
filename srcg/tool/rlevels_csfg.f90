...
!     NIST                                                 May 2011    *
!                                                                      *
! Some mirror modifications by Chongyang Chen, Fudan university        *
! In addition, the code is modified to read partially calculated       *
! <state>.(c)m                                                         *
!                                                          Dec 2020    *
!***********************************************************************
      IMPLICIT NONE
...
!
      ...
      CHARACTER g92mix*6
      CHARACTER*128 string_CSF(ndim), string_PRN(ndim) ! String in LSJ
      CHARACTER util_lbl_file*256
!CYC:
      INTEGER Lev_POS_MCM(ndim)
!
...
!
         ...
         DO jblock = 1, nblock
            READ (3, end=231) nb, ncfblk, nevblk, iiatjp, iiaspa
...
!
            ...
            READ (3, err=231) (ivec(i+ncountState), i = 1, nevblk)
            READ (3, err=231) eav, (eval(i+ncountState), i = 1, nevblk)
            READ (3, err=230) (evec, i = 1, ncfblk*nevblk)
!
!     Assign J and parity to every individual state
!     Also add the average energy (back) to energy
!
230         DO i = 1, nevblk
               !iatjp(i+ncountState)=ADJUSTL(ADJUSTR(JFraction(iiatjp)))
               iatjp(i+ncountState)=JFraction(iiatjp)
               iaspa(i+ncountState) = PlusMinus(iiaspa)
               eval(i+ncountState) = eval(i+ncountState) + eav
               Lev_POS_MCM(i+ncountState) = i
            END DO
!
!     Update ncountState
!
...
!
         ...
         do i = 1,ncountState
            do j = 1,IMaxCount
! CYC: in the cases of energies are same for different levels,
! such as 1s2 2s2 7h_{11/2} and 7i_{11/2} in C II
                if (Lev_POS_MCM(i) .EQ. Lev_POS(j)   .AND. &
                   ADJUSTL(ADJUSTR(iatjp(i)))        .EQ.  &
                   ADJUSTL(ADJUSTR(Lev_J(j)))        .AND. & 
                   iaspa(i).EQ.Lev_Par(j)) then
                   if (DABS(DABS(eval(i))-DABS(RLev_ENER(j))).gt.1.0d-7)then
                     write(*,*)"Warning, Large energy deviations ..."
                     write(*,*)"eval(i)=",eval(i)
                     write(*,*)"RLev_ENER(j)=",RLev_ENER(j)
                   endif   
...
!
      ...
      READ (31,'(1X,I2,1X,A4,5X,A1,8X,F16.9)',IOSTAT = IOS)  &
        ...
        RLev_ENER(ICount)
        !Lev_J(ICount)=ADJUSTL(ADJUSTR(Lev_J(ICount)))
      IF (IOS .NE. 0) GO TO 1
!
